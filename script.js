...
//DOM
const btnForFree=document.querySelector("#btn_forFree");
const btnForPaid=document.querySelector("#btn_forPaid");
const freeData = document.querySelector("#freeData");  //要顯示出來的免費停車資料
const paidData = document.querySelector("#paidData");  //要顯示出來的付費停車資料

//點擊事件(for免費的)
btnForFree.addEventListener("click", function(e){  
    let strFree = "";
    ...
    freeData.innerHTML = strFree;
    console.log("freeData generated")
});

//點擊事件(for投幣式的)

btnForPaid.addEventListener("click", 
  function(){
    let strpaid="";
    let paidDataArr =[];
    ...
    data.forEach(function(item, index){
        if(item.Charge === "投幣式"){
          // total.paid ++;
          paidDataArr.push(item);
        };
    });

    paidDataArr.forEach(function(item, index){
      // displayFreeLocation.push(newData.free[i].Location);
      // displayFreeAddress.push(newData.free[i].Address);

      //用DOM推進去變成表單規格
      strpaid += `<tr>
            <td>${paidDataArr[index].Location}</td>
            <td>${paidDataArr[index].Address}</td>
      </tr>`
    });
    paidData.innerHTML = strpaid; 
   console.log("paidData generated!")
  });    


//   let displayFreeLocation = [];
//   let displayFreeAddress = [];